...
; _EN(`call')_RU(`вызов')_ES(`llamada a') puts():
...
; _EN(`call')_RU(`вызов')_ES(`llamada a') scanf():
  4006e4:       8fbc0010        lw      gp,16(sp)
  4006e8:       3c040040        lui     a0,0x40
  4006ec:       8f998038        lw      t9,-32712(gp)
; _EN(`prepare address of')_RU(`подготовить адрес')_ES(`preparar la dirección de') x:
...
; _EN(`get address of')_RU(`взять адрес')_ES(`obtener la dirección de') x:
  400704:       8f828044        lw      v0,-32700(gp)
  400708:       8f99803c        lw      t9,-32708(gp)
; _EN(`load value from "x" variable and pass it to')_RU(`загрузить значение из переменной "x" и передать его в')_ES(`cargar el valor de la variable "x" y pasarlo a') printf() _in $a1:
...
; _EN(`pack of NOPs used for aligning next function start on 16-byte boundary')_RU(`набор NOP-ов для выравнивания начала следующей ф-ции по 16-байтной границе')_ES(`conjunto de NOPs utilizados para alinear el inicio de la siguiente función en un límite de 16 bytes'):
