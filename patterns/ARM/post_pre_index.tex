\section{\RU{Режимы адресации}\EN{Addressing modes}\ES{Modos de direccionamiento}}
\index{ARM!\RU{Режимы адресации}\EN{Addressing modes}\ES{Modos de direccionamiento}}
...
\ES{Esta instrucción es posible en ARM64:}
...
\ES{Esto significa cargar el valor en la dirección contenida en X1 y luego sumar 28 a X1.}
...
\ES{ARM permite sumar o restar una constante a/de la dirección usada para la carga.}
\RU{Причем, позволяет это делать до загрузки или после.}
\EN{And it's possible to do that both before and after loading.}
\ES{Y es posible hacerlo tanto antes como después de la carga.}
...
\ES{Este modo de direccionamiento no existe en x86, pero sí en otros procesadores, incluso en el PDP-11.}
...
\ES{Por cierto, esta es una característica de C difícil de recordar.}
\RU{Дела обстоят так:}\EN{This is how it is:}\ES{Así es como funciona:}

% FIXME: add ARM assembly...
\begin{center}
\begin{tabular}{ | l | l | l | l | }
\hline
\headercolor{} \RU{термин в Си}\EN{C term}\ES{Término en C} & 
\headercolor{} \RU{термин в ARM}\EN{ARM term}\ES{Término en ARM} & 
\headercolor{} \RU{выражение Си}\EN{C statement}\ES{Expresión en C} & 
\headercolor{} \RU{как это работает}\EN{how it works}\ES{cómo funciona} \\
\hline
\PostIncrement & 
post-indexed addressing & 
\TT{*ptr++} & 
\RU{использовать значение \TT{*ptr}}\EN{use \TT{*ptr} value}\ES{usar el valor de \TT{*ptr}}, \\
& & & \RU{затем инкремент указателя \TT{ptr}}\EN{then \gls{increment} \TT{ptr} pointer}\ES{luego \gls{increment} el puntero \TT{ptr}} \\
\hline
\PostDecrement & 
post-indexed addressing & 
\TT{*ptr-{}-} & 
\RU{использовать значение \TT{*ptr}}\EN{use \TT{*ptr} value}\ES{usar el valor de \TT{*ptr}}, \\
& & & \RU{затем \glslink{decrement}{декремент} указателя \TT{ptr}}\EN{then \gls{decrement} \TT{ptr} pointer}\ES{luego \gls{decrement} el puntero \TT{ptr}} \\
\hline
\PreIncrement & 
pre-indexed addressing & 
\TT{*++ptr} & 
\RU{инкремент указателя \TT{ptr}}\EN{\gls{increment} \TT{ptr} pointer}\ES{\gls{increment} el puntero \TT{ptr}}, \\
& & & \RU{затем использовать значение \TT{*ptr}}\EN{then use \TT{*ptr} value}\ES{luego usar el valor de \TT{*ptr}} \\
\hline
\PreDecrement & 
pre-indexed addressing & 
\TT{*-{}-ptr} & 
\RU{\glslink{decrement}{декремент} указателя \TT{ptr}}\EN{\gls{decrement} \TT{ptr} pointer}\ES{\gls{decrement} el puntero \TT{ptr}}, \\
& & & \RU{затем использовать значение \TT{*ptr}}\EN{then use \TT{*ptr} value}\ES{luego usar el valor de \TT{*ptr}} \\
\hline
\end{tabular}
\end{center}
...
восклицательный знак в ассемблере ARM}\ES{se marca con un signo de exclamación en el lenguaje ensamblador de ARM}.
\RU{Для примера, смотрите строку 2 в}\EN{For example, see line 2 in}\ES{Por ejemplo, ver la línea 2 en} \lstref{hw_ARM64_GCC}.
...
\ES{Dennis Ritchie (uno de los creadores del lenguaje C) mencionó que probablemente fue idea de Ken Thompson (otro creador de C), porque esta característica del procesador ya estaba presente en el PDP-7}
...
\ES{Por lo tanto, los compiladores de C pueden usarlo si está presente en el procesador de destino.}

\RU{Всё это очень удобно для работы с массивами.}
\EN{That's very convenient for array processing.}
\ES{Todo esto es muy conveniente para trabajar con arreglos.}
