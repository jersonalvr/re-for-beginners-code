...
Windows}\ES{es un formato de archivo ejecutable usado en Windows}.

\RU{Разница между .exe, .dll, и .sys в том, что у .exe и .sys обычно нет экспортов, только импорты}
\EN{The difference between .exe, .dll and .sys is that .exe and .sys usually do not have exports, only imports}\ES{La diferencia entre .exe, .dll y .sys es que .exe y .sys normalmente no tienen exportaciones, solo importaciones}.
...
but this function usually does nothing.}\ES{Una \ac{DLL}, igual que cualquier otro archivo PE, tiene un punto de entrada (\ac{OEP})
(allí está la función DllMain()), pero normalmente esa función no hace nada.}

.sys \RU{это обычно драйвера устройств}\EN{is usually a device driver}\ES{normalmente es un controlador de dispositivo}.
...
\ES{Para los controladores, Windows exige que la suma de comprobación esté presente en el archivo PE y sea correcta}
\footnote{\RU{Например}\EN{For example}\ES{Por ejemplo}, Hiew(\myref{Hiew}) \RU{умеет её подсчитывать}\EN{can calculate it}\ES{puede calcularla}}.
...
\ES{Desde Windows Vista, los archivos de los controladores también deben ir firmados digitalmente; de lo contrario no se cargarán.}
...
this message will be printed.}\ES{Al comienzo de cualquier archivo PE hay un pequeño programa DOS que muestra un mensaje como} \q{This program cannot be run in DOS mode.}\EMDASH{}%
\ES{si se ejecuta en DOS o Windows 3.1 (\ac{OS} que no conocen el formato PE), se mostrará ese mensaje.}

\subsection{\RU{Терминология}\EN{Terminology}}

\begin{itemize}
\item
\RU{Модуль}\EN{Module}\ES{Módulo}\EMDASH{}\RU{это отдельный файл}\EN{a separate file}\ES{es un archivo independiente}, .exe \OrENRU{} .dll.

\item
  \RU{Процесс}\EN{Process}\ES{Proceso}\EMDASH{}\RU{это некая загруженная в память и работающая программа}\EN{a program
loaded into memory and currently running}\ES{es un programa cargado en memoria y ejecutándose}.
\RU{Как правило состоит из одного .exe-файла и массы .dll-файлов}\EN{Commonly consists of 
one .exe file and bunch of .dll files}\ES{Normalmente consta de un archivo .exe y un montón de archivos .dll}.

\item
  \RU{Память процесса}\EN{Process memory}\ES{Memoria del proceso}\EMDASH{}\RU{память с которой работает процесс}\EN{the memory a process
works with}\ES{la memoria con la que trabaja el proceso}.
\RU{У каждого процесса\EMDASH{}своя}\EN{Each process has its own}\ES{Cada proceso tiene la suya}.
\RU{Там обычно имеются загруженные модули, память стека, \glslink{heap}{кучи},}
\EN{There usually are loaded modules, memory of the stack, \gls{heap}(s),}\ES{Suele haber módulos cargados, memoria de la pila, \gls{heap}(s),}\etc{}.

\item
\index{VA}
\ac{VA}\EMDASH{}\RU{это адрес, который будет использоваться в самой программе во время исполнения.}
\EN{an address which is to be used in program while runtime.}\ES{es una dirección que se usa en el propio programa en tiempo de ejecución.}
...
\ES{es la dirección en la memoria del proceso donde debe cargarse el módulo. El cargador de la \ac{OS} puede cambiarla si ese destino ya está ocupado por otro módulo cargado antes.}

\item
\index{RVA}
\ac{RVA}\EMDASH{}\RU{это}\EN{the} \ac{VA}-\RU{адрес минус базовый адрес}\EN{address minus the base address}.\ES{es la dirección \ac{VA} menos la dirección base.}
\RU{Многие адреса в таблицах PE-файла используют}
\EN{Many addresses in PE-file tables use}
\ac{RVA}-\RU{адреса}\EN{addresses}\ES{Muchas direcciones en las tablas de un archivo PE usan direcciones \ac{RVA}}.
...
\item 
\index{Windows!IAT}
\ac{IAT}\EMDASH{}\RU{массив адресов импортированных символов}\EN{an array of addresses of imported symbols}\ES{un arreglo de direcciones de símbolos importados}
\footnote{\cite{Pietrek1}}. 
\RU{Иногда, директория}\EN{Sometimes, the} \TT{IMAGE\_DIRECTORY\_ENTRY\_IAT} \RU{указывает на}
\EN{data directory points at the} \ac{IAT}\ES{A veces, la entrada de directorio de datos \TT{IMAGE\_DIRECTORY\_ENTRY\_IAT} apunta a la \ac{IAT}}. 
\label{IDA_idata}
...
\EN{is a part of another section}!\ES{Es importante notar que \ac{IDA} (al menos hasta la 6.1) puede crear una seudosección llamada \TT{.idata} para la \ac{IAT}, incluso si la \ac{IAT} forma parte de otra sección}!

\item 
\index{Windows!INT}
\ac{INT}\EMDASH{}\RU{массив имен символов для импортирования}
\EN{an array of names of symbols to be imported}\ES{un arreglo con los nombres de los símbolos a importar}\footnote{\cite{Pietrek1}}.
\end{itemize}
...
to reach an agreement which addresses is to be assigned to whose modules.}\ES{El problema es que varios autores de módulos pueden preparar DLL para que otros las usen y no es posible acordar qué direcciones se asignan a quién.}
...
and each virtual addresses in the second DLL will be corrected.}\ES{Por eso, si dos DLL necesarias para un proceso tienen la misma dirección base, una se cargará en esa base y la otra en otro espacio libre de la memoria del proceso, y todas las direcciones virtuales de la segunda DLL se ajustarán.} \\
\\
...
\footnote{\RU{Это можно изменять опцией /BASE в линкере}\EN{This can be changed by the /BASE linker option}}\ES{Con frecuencia, el enlazador de \ac{MSVC} genera archivos .exe con dirección base \TT{0x400000} y la sección de código arrancando en \TT{0x401000}; esto significa que el \ac{RVA} del inicio de la sección de código es \TT{0x1000}. Las \ac{DLL} suelen generarse con base \TT{0x10000000}, y se puede cambiar con la opción /BASE del enlazador}.

\index{ASLR}
\RU{Помимо всего прочего, есть еще одна причина намеренно загружать модули по разным адресам, а точнее, 
по случайным}
\EN{There is also another reason to load modules at various base addresses, in this case random ones}\ES{Además hay otra razón para cargar los módulos en direcciones distintas, concretamente en direcciones aleatorias}.

\RU{Это}\EN{It is} \ac{ASLR}\footnote{\RU{\href{http://go.yurichev.com/17042}{wikipedia}}\EN{\href{http://go.yurichev.com/17140}{wikipedia}}}\ES{Esto es} \ac{ASLR}.
...
\EN{A shellcode trying to get executed on a compromised system must call system functions, hence, know their addresses.}\ES{Un shellcode que intenta ejecutarse en un sistema comprometido debe llamar funciones del sistema y, por lo tanto, conocer sus direcciones}.
...
fixed and shellcode could call them directly}\ES{las DLL del sistema (como kernel32.dll o user32.dll) se cargaban siempre en direcciones conocidas y, si recordamos que sus versiones cambiaban rara vez, las direcciones de las funciones eran prácticamente fijas y el shellcode podía llamarlas directamente}.
...
\EN{method loads your program and all modules it needs at random base addresses, different every time}\ES{carga tanto tu programa como todos los módulos que necesita en direcciones base aleatorias, diferentes en cada ejecución}.

\RU{В PE-файлах, поддержка \ac{ASLR} отмечается выставлением флага}
\EN{\ac{ASLR} support is denoted in a PE file by setting the flag}\ES{El soporte de \ac{ASLR} en un archivo PE se marca activando el indicador} \\
\TT{IMAGE\_DLL\_CHARACTERISTICS\_DYNAMIC\_BASE} \cite{Russinovich}.

\subsection{Subsystem}

\RU{Имеется также поле \IT{subsystem}, обычно это}\EN{There is also a \IT{subsystem} field, usually it is}\ES{También hay un campo \IT{subsystem}, que suele ser}:

\index{Native API}
\begin{itemize}
\item native\footnote{\EN{Meaning, the module use Native API instead of Win32}\RU{Что означает, что модуль использует Native API а не Win32}\ES{Significa que el módulo usa Native API en lugar de Win32}} 
(.sys-\RU{драйвер}\EN{driver}\ES{controlador}), 

\item console (\RU{консольное приложение}\EN{console application}\ES{aplicación de consola}) \OrENRU 

\item \ac{GUI} (\RU{не консольное}\EN{non-console}\ES{no es de consola}).
\end{itemize}
...
\ES{El archivo PE también especifica el número mínimo de versión de Windows necesario para cargar el módulo.}
...
\RU{здесь}\EN{here}\ES{aquí}\footnote{\href{http://go.yurichev.com/17044}{wikipedia}}.
...
\RU{для запуска необходима как минимум Windows 2000}\EN{at least Windows 2000 is needed to run them}\ES{los archivos generados tienen versión 5.00 y requieren al menos Windows 2000 para ejecutarse}).
...
\footnote{\href{http://go.yurichev.com/17045}{MSDN}},
it is possible to force it to compile for Windows XP.}\ES{Por defecto, \ac{MSVC} 2012 genera archivos .exe de versión 6.00, que necesitan al menos Windows Vista; sin embargo, cambiando las opciones del compilador\footnote{\href{http://go.yurichev.com/17045}{MSDN}} es posible forzar la generación para Windows XP.}
...
\EN{the flags}\ES{En una sección de datos constantes (protegida contra escritura) pueden estar los indicadores} \\
...
section}\ES{Si se intenta escribir algo en esa sección, el proceso fallará}.
\end{itemize}

\RU{В PE-файле можно задавать название для секции, но это не важно}\EN{Each section in PE-file may have a name, however,
it is not very important}\ES{En un archivo PE se puede asignar un nombre a cada sección, aunque no es algo crucial}.
\RU{Часто (но не всегда)}\EN{Often (but not always)} \RU{секция кода называется}\EN{the code section is named} \TT{.text}\ES{la sección de código se llama} \TT{.text}, 
\index{TLS}
\index{BSS}
\RU{секция данных}\EN{the data section}\EMDASH{}\TT{.data}, \RU{константных данных}\EN{the constant data section} --- \TT{.rdata} 
\IT{(readable data)}\ES{(datos legibles)}.
\RU{Еще популярные имена секций}\EN{Other popular section names are}\ES{Otros nombres comunes de secciones son}: 

\index{MIPS}
\begin{itemize}
\item \TT{.idata}\EMDASH{}\RU{секция импортов}\EN{imports section}\ES{sección de importaciones}.
\ac{IDA} \RU{может создавать псевдо-секцию с этим же именем}
\EN{may create a pseudo-section named like this}\ES{puede crear una seudosección con el mismo nombre}: \myref{IDA_idata}.
\item \TT{.edata}\EMDASH{}\RU{секция экспортов (редко встречается)}\EN{exports section (rare)}\ES{sección de exportaciones (poco común)}
\item \TT{.pdata}\EMDASH{}\RU{секция содержащая информацию об исключениях в Windows NT для MIPS, \ac{IA64} и x64}
\EN{section containing all information about exceptions in Windows NT for MIPS, \ac{IA64} and x64}\ES{sección que contiene toda la información sobre excepciones en Windows NT para MIPS, \ac{IA64} y x64}: \myref{SEH_win64}
\item \TT{.reloc}\EMDASH{}\RU{секция релоков}\EN{relocs section}\ES{sección de relocs}
\item \TT{.bss}\EMDASH{}\RU{неинициализированные данные}\EN{uninitialized data (\ac{BSS})}\ES{datos no inicializados (\ac{BSS})}
\item \TT{.tls}\EMDASH{}thread local storage (\ac{TLS})\ES{almacenamiento local de hilo (\ac{TLS})}
\item \TT{.rsrc}\EMDASH{}\RU{ресурсы}\EN{resources}\ES{recursos}
\item \TT{.CRT}\EMDASH{}\RU{может присутствует в бинарных файлах, скомпилированных очень старыми версиями MSVC}
\EN{may present in binary files compiled by ancient MSVC versions}\ES{puede aparecer en binarios compilados con versiones muy antiguas de MSVC}
\end{itemize}

\RU{Запаковщики/зашифровщики PE-файлов часто затирают имена секций, или меняют на свои}
\EN{PE file packers/encryptors often garble section names or replace the names with their own}\ES{Los empaquetadores o cifradores de archivos PE suelen borrar los nombres de las secciones o cambiarlos por los suyos propios}.

\RU{В \ac{MSVC} можно объявлять данные в произвольно названной секции}
\EN{\ac{MSVC} allows you to declare data in arbitrarily named section}\ES{En \ac{MSVC} se pueden declarar datos en una sección con nombre arbitrario}
...
other debugging information (MinGW for instance).}\ES{Algunos compiladores y enlazadores pueden añadir también una sección con símbolos de depuración y, en general, información de depuración (por ejemplo, MinGW).}
\index{Windows!PDB}
...
\EN{separate \gls{PDB} files are used there for this purpose})\ES{allí se suele guardar la información de depuración en archivos \gls{PDB} separados}.\\
\\
\RU{Вот как PE-секция описывается в файле}\EN{That is how a PE section is described in the file}\ES{Así se describe una sección PE en el archivo}:
...
\index{Hiew}
\RU{Еще немного терминологии}\EN{A word about terminology}\ES{Algo más de terminología}:
\IT{PointerToRawData} \RU{называется}\EN{it called} \q{Offset} \InENRU Hiew
\AndENRU \IT{VirtualAddress} \RU{называется}\EN{is called} \q{RVA} \RU{там же}\EN{there}\ES{allí}.
