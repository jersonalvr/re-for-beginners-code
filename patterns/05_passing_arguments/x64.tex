...
\ES{La historia es un poco diferente en x86-64. Los argumentos de la función (los primeros 4 o los primeros 6 de ellos) se pasan en registros, es decir, la función llamada (\gls{callee}) los lee de los registros en lugar de leerlos de la pila.}
...
\ES{Como se puede ver, la compacta función \ttf toma todos sus argumentos directamente de los registros.}
...
\ES{La instrucción \LEA se utiliza aquí para la suma, aparentemente el compilador la consideró más rápida que \TT{ADD}.}
...
\ES{\LEA también se utiliza en la función \main para preparar el primer y tercer argumento de \ttf. El compilador debe haber decidido que esto funcionaría más rápido que la forma habitual de cargar valores en un registro mediante la instrucción \MOV.}

\RU{Попробуем посмотреть вывод неоптимизирующего MSVC}\EN{Let's take a look at the non-optimizing MSVC output}:
\ES{Echemos un vistazo a la salida de MSVC sin optimización}:
...
\ES{Parece un poco desconcertante porque, por alguna razón, los 3 argumentos de los registros se guardan en la pila.}
\index{Shadow space}
\label{shadow_space}
...
\ES{Esto se llama} \q{shadow space} \footnote{\href{http://go.yurichev.com/17256}{MSDN}}: \ES{cada función en Win64 puede (aunque no está obligada a) guardar los valores de los 4 registros allí.}
\RU{Это делается по крайней мере из-за двух причин}\EN{This is done for two reasons}: 
\ES{Esto se hace al menos por dos razones}:
...
\ES{1) en una función grande, asignar un registro entero (o incluso 4 registros) para un argumento de entrada es demasiado costoso, por lo que se accederá a él a través de la pila;}
...
\ES{2) el depurador siempre sabe dónde encontrar los argumentos de la función en un punto de interrupción}\footnote{\href{http://go.yurichev.com/17257}{MSDN}}.
...
\ES{Por lo tanto, algunas funciones grandes pueden guardar sus argumentos de entrada en el \q{shadow space} si necesitan usarlos durante la ejecución, pero algunas funciones pequeñas (como la nuestra) pueden no hacerlo.}
...
\ES{Es responsabilidad de la función de llamada (\gls{caller}) asignar el \q{shadow space} en la pila.}

\ifdefined\IncludeGCC
\subsection{GCC}

\Optimizing GCC \RU{также делает понятный код}\EN{generates more or less understandable code}:
\ES{también genera un código más o menos comprensible}:
...
\ES{No existen requisitos de \q{shadow space} en System V *NIX\cite{SysVABI}, pero la función llamada (\gls{callee}) puede necesitar guardar sus argumentos en algún lugar en caso de escasez de registros.}

\subsection{GCC: uint64\_t \RU{вместо}\EN{instead of}\ES{en lugar de} int}
...
\ES{Nuestro ejemplo trabaja con un \Tint de 32 bits, por lo que se utilizan las partes de registro de 32 bits (con el prefijo \TT{E-}).}
...
\ES{Se puede modificar ligeramente para usar valores de 64 bits}:
...
\ES{El código es el mismo, pero esta vez se utilizan los registros en su \IT{tamaño completo} (con el prefijo \TT{R-}).}
\fi
