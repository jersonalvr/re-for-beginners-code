...
\subsubsection{\EN{Uninitialized global variable}\RU{Неинициализированная глобальная переменная}\ES{Variable global no inicializada}}

\RU{Так что теперь переменная $x$ глобальная.}
\EN{So now the $x$ variable is global.}
\ES{Así que ahora la variable $x$ es global.}
...
\ES{Compilemos a un archivo ejecutable en lugar de un archivo objeto y carguémoslo en \IDA.}
...
\ES{IDA muestra la variable $x$ en la sección ELF .sbss (¿recuerda el \q{Global Pointer}? \myref{MIPS_GP}), ya que la variable no se inicializa al principio.}
...
\ES{IDA reduce la cantidad de información, por lo que también haremos un listado usando objdump y lo comentaremos:}
...
\ES{Ahora vemos que la dirección de la variable $x$ se lee de un búfer de datos de 64KiB usando GP y sumándole un desplazamiento negativo (línea 18).}
...
\ES{Más que eso, las direcciones de las tres funciones externas que se utilizan en nuestro ejemplo (\puts, \scanf, \printf) también se leen del búfer de datos global de 64KiB usando GP (líneas 9, 16 y 26).}
...
\ES{GP apunta al medio del búfer, y dicho desplazamiento sugiere que las direcciones de las tres funciones, y también la dirección de la variable $x$, están almacenadas en algún lugar al principio de ese búfer.}
\RU{Действительно, ведь наш пример крохотный}\EN{That make sense, because our example is tiny}.
\ES{Esto tiene sentido, porque nuestro ejemplo es pequeño.}
...
\ES{Otra cosa que vale la pena mencionar es que la función termina con dos \ac{NOP}s (\TT{MOVE \$AT,\$AT} --- una instrucción inactiva), para alinear el inicio de la siguiente función en un límite de 16 bytes.}

\subsubsection{\RU{Инициализированная глобальная переменная}\EN{Initialized global variable}\ES{Variable global inicializada}}
...
\ES{Modifiquemos nuestro ejemplo dándole a la variable $x$ un valor por defecto:}
...
\ES{Ahora IDA muestra que la variable $x$ reside en la sección .data:}
...
\ES{¿Por qué no en .sdata? ¿Tal vez esto dependa de alguna opción de GCC?}
...
\ES{Sin embargo, ahora $x$ está en .data, que es un área de memoria general, y podemos echar un vistazo a cómo se trabaja con las variables allí.}
...
\ES{La dirección de la variable debe formarse mediante un par de instrucciones.}
...
\ES{En nuestro caso son LUI (\q{Load Upper Immediate}) y ADDIU (\q{Add Immediate Unsigned Word}).}
...
\ES{Aquí está también el listado de objdump para una inspección detallada:}
...
\ES{Vemos que la dirección se forma usando LUI y ADDIU, pero la parte alta de la dirección todavía está en el registro \$S0, y es posible codificar el desplazamiento en una instrucción LW (\q{Load Word}), de modo que una sola LW es suficiente para cargar un valor de la variable y pasarlo a \printf.}
...
\ES{Los registros que contienen datos temporales tienen el prefijo T-, pero aquí también vemos algunos con el prefijo S-, cuyos contenidos deben preservarse antes de su uso en otras funciones (es decir, \q{saved}).}
% FIXME:
...
\ES{Es por eso que el valor de \$S0 se estableció en la dirección 0x4006cc y se usó nuevamente en la dirección 0x4006e8, después de la llamada a \scanf.}
\RU{Функция \scanf не изменяет это значение.}\EN{The \scanf function does not change its value.}
\ES{La función \scanf no cambia su valor.}
