...
\RU{Тут всё просто}\EN{Everything here is simple}\ES{Todo aquí es simple}.
...
\ES{\TT{MADD} es simplemente una instrucción que realiza una multiplicación/adición fusionada (similar a \TT{MLA} que ya vimos).}
...
\ES{También extendamos todos los tipos de datos a \TT{uint64\_t} de 64 bits y probemos:}
...
\ES{La función \ttf{} es la misma, solo que ahora se utilizan los registros X de 64 bits completos.}
...
\ES{Los valores largos de 64 bits se cargan en los registros por partes, esto también se describe aquí: \myref{ARM_big_constants_loading}.}
...
\ES{El compilador sin optimización es más redundante:}
...
\ES{El código guarda sus argumentos de entrada en la pila local, en caso de que alguien (o algo) en esta función necesite utilizar los registros \TT{W0...W2}. Esto evita la sobreescritura de los argumentos originales de la función, que podrían ser necesarios nuevamente en el futuro.}
\RU{Это называется}\EN{This is called}\ES{Esto se llama} \IT{Register Save Area.} \cite{ARM64_PCS}
\RU{Вызываемая функция не обязана сохранять их.}\EN{ The callee, however, is not obliged to save them.}
\ES{Sin embargo, la función llamada (\gls{callee}) no está obligada a guardarlos.}
\RU{Это то же что и}\EN{This is somewhat similar to}\ES{Esto es algo similar a} \q{Shadow Space}: \myref{shadow_space}.
...
\ES{¿Por qué el GCC 4.9 optimizado descartó este código de guardado de argumentos?}
...
\ES{Porque realizó un trabajo de optimización adicional y concluyó que los argumentos de la función no serán necesarios en el futuro y también que los registros \TT{W0...W2} no serán utilizados.}
...
\ES{También vemos un par de instrucciones \TT{MUL}/\TT{ADD} en lugar de una sola \TT{MADD}.}
