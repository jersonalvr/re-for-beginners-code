...
\RU{Теперь попробуем использовать scanf().}\EN{Now let's use scanf().}\ES{Ahora intentemos usar scanf().}
...
Почему?}
\ES{Este código, compilado en Linux x86-64 usando GCC, falla durante la ejecución (fallo de segmentación).
Sin embargo, funciona en un entorno de Windows compilado por MSVC 2010 x86.
¿Por qué?}
