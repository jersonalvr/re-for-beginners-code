\section{\RU{Глобальные переменные}\EN{Global variables}\ES{Variables globales}}
\index{\RU{Глобальные переменные}\EN{Global variables}\ES{Variables globales}}
...
\ES{¿Qué pasaría si la variable \TT{x} del ejemplo anterior fuera global en lugar de local? Entonces se podría acceder a ella desde cualquier lugar, no solo desde el cuerpo de la función. Las variables globales se consideran un \glslink{anti-pattern}{antipátrón}, pero para fines del experimento, podemos permitirnos esto.}
